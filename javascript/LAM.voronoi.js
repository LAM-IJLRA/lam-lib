...
var VoronoiDemo = {
    ...
    init: function() {
        ...
        this.randomSites(200,true);
        //this.render();
		post("edges:"+ JSON.stringify(this.diagram.edges) );
		post("\n");
		//outlet(0, JSON.stringify(this.diagram.edges));
		post("sites:"+ JSON.stringify(this.sites));
		post("\n");
		
		post("vertices:"+ JSON.stringify(this.diagram.vertices) );
		post("\n");
    ...
    randomSites: function(n,clear) {
        if (clear) {this.sites = [];}
        // create vertices
        var xmargin = 800*this.margin,//this.canvas.width*this.margin,
            ymargin = 600*this.margin,//this.canvas.height*this.margin,
            xo = xmargin,
            dx = 800;//this.canvas.width-xmargin*2,
            yo = ymargin,
            dy = 600;//this.canvas.height-ymargin*2;
        for (var i=0; i<n; i++) {
            this.sites.push({
...
{
	var siteArray = [];
	outlet(0, "sites", "length", VoronoiDemo.sites.length);
...
function getVertices()
{
	...
	var verticesArray = [];
	var myVertices = VoronoiDemo.diagram.vertices;
	
	outlet(0, "vertices", "length", myVertices.length);
	for (var i = 0; i < myVertices.length; i++) {
    	//alert(myStringArray[i]);
    	//Do something
		verticesArray.push(myVertices[i].x, myVertices[i].y);
		//outlet(0, VoronoiDemo.sites[i].voronoiId,VoronoiDemo.sites[i].x, VoronoiDemo.sites[i].y);
	}
	outlet(0, "vertices", "data", verticesArray);
}
...
function getEdges()
{
	...
	var edgesArray = [];
	var myEdges = VoronoiDemo.diagram.edges;
	
	outlet(0, "edges", "length", myEdges.length);
	for (var k = 0; k < myEdges.length; k++) {
    	//alert(myStringArray[i]);
    	//Do something
		edgesArray.push(myEdges[k].va.x, myEdges[k].va.y, myEdges[k].vb.x, myEdges[k].vb.y);
		//outlet(0, VoronoiDemo.sites[i].voronoiId,VoronoiDemo.sites[i].x, VoronoiDemo.sites[i].y);
	}
	outlet(0, "edges", "data", edgesArray);
}
...
{ 
	...
	var x;
	var y;
	var siteList = a.slice(1);
	
	VoronoiDemo.sites= [];
	//post(a.slice(1) + "\n");
	for (var i=0; i<(siteList.length/2); i++) {
    	VoronoiDemo.sites.push({
        	x: siteList[i*2],
            y: siteList[(i*2)+1]
		});
    }
...
function getFirstSiteCell(){

	//post(JSON.stringify(VoronoiDemo.diagram)+"\n");
	...
	if (cell) {
			...
			if (nHalfedges > 2) {
				...
				outlet(0, "cell0", "data", hE_list);
			}
	}
}

function getFirstSiteCellArea(){
